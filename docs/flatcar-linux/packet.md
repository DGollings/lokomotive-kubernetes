...
| cluster_domain_suffix | FQDN suffix for Kubernetes services answered by coredns. | "cluster.local" | "k8s.example.com" |
| service_cidr | CIDR IPv4 range to assign to Kubernetes services | "10.3.0.0/16" | "10.3.0.0/24" |
| setup_raid | Create a RAID 0 from extra (i.e. non-OS) disks on a Packet node | false | true |
| setup_raid_hdd    | Create a RAID 0 from extra (i.e. non-OS) hard disk drives (HDD) only. Has no effect if `setup_raid` is `true`   | false | true |
| setup_raid_ssd    | Create a RAID 0 from extra (i.e. non-OS) solid state drives (SSD) only. Has no effect if `setup_raid` is `true` | false | true |
| setup_raid_ssd_fs | Create a file system on the RAID 0 created using the `setup_raid_ssd` variable. Has no effect if `setup_raid` is `true` | true | false |
